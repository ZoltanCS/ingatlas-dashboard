...
export default function TeamPage() {
  ...
  useEffect(() => {
    getTeamMembers().then(m => { setMembers(m); setLoading(false) })
  }, [])
  ...
  return (
    <>
      <Helmet>
        <title>Csapat — Dashboard | Ingatlas</title>
      </Helmet>
          ...
          {members.map((m, i) => (
            <div
              ...
              style={{
                ...
                padding: '12px 20px',
                borderBottom: i < members.length - 1 ? '0.5px solid rgba(0,0,0,0.04)' : 'none',
              }}
            >
              ...
              <span style={{ flex: 1 }}>
                <span style={{
                  ...
                  padding: '3px 8px',
                  borderRadius: '3px',
                  background: ROLE_STYLES[m.role]?.bg || '#f0f0f1',
                  color: ROLE_STYLES[m.role]?.color || '#999',
                  fontFamily: '"JetBrains Mono", monospace',
                  textTransform: 'uppercase',
                  letterSpacing: '0.06em',
                }}>{m.role}</span>
              </span>
              <span style={{ flex: 1, fontSize: '11px', color: '#bbb' }}>
                {new Date(m.joined).toLocaleDateString('hu')}
              </span>
